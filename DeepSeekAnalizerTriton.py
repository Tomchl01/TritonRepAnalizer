...
MAX_INPUT_TOKENS = 55000  # Define max input tokens, make it a constant
TIMEOUT = 60  # Global timeout for requests, in seconds
RETRIES = 5    # Global number of retries
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("deepseek_analysis.log", encoding='utf-8'), #Log to file using utf-8
        logging.StreamHandler() #Log to console
    ],
    encoding='utf-8' #Set global encoding for logging
)
    ...
    if not os.path.exists(file_path):
        try:
            with open(file_path, 'w', encoding='utf-8') as f:
                json.dump(default_data, f, indent=4)  # Create with indent
            return default_data
        except OSError as e:
        ...
        try:
            ...
            return None # Or return default_data, to overwrite.
...
def make_deepseek_request(url, data, api_key, timeout=TIMEOUT, retries=RETRIES):
    ...
    session.mount("http://", adapter)  # In case you use HTTP
...
"""
    data = {
        ...
        "max_tokens": 8192  # Or a smaller value if appropriate
    }
    ...
    if response_data:
        try:
            ...
            logging.error(f"Response data: {response_data}") #Log response to debug
            return None
    else:
        # Request failed (handled in make_deepseek_request), so return None
        return None
...
def main():
    ...
    if processed_videos is None:
        ...
        return  # Exit if we can't load/create the processed videos file.
    ...
    for filename in os.listdir(TRANSCRIPT_DIR):
        ...
        data = load_or_create_json(file_path, None) #Use default data
        if data is None:
            logging.error(f"Skipping {filename} due to load error.")
            continue  # Skip this file if it can't be loaded

        video_id = data.get("video_id")  # Use .get() for safety
        if not video_id:
        ...
        last_log_time = start_time #Initialize to calculate ETA properly
        ...
        for idx, chunk in enumerate(tqdm(chunks, desc="Summarizing", unit="chunk", dynamic_ncols=True), start=1):
            ...
            # Calculate and log ETA every 10 seconds, or on the last chunk
        ...
        try:
            ...
            continue #Try saving to the next file, we don't stop entirely.


        # Update processed videos *after* successful summary save
        ...
        try:
            ...
            #Consider what else to do here.  Maybe write to another temporary file.
            #For now, continue.
